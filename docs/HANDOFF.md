...
- Mobile-first tap controls: tapping the reader text toggles compact floating controls. The lightly polished layout uses top controls for Preview, Home, and Mode; a small centered Chapters/progress status line; and bottom Previous/Next navigation. Interactive targets such as vocabulary terms, bubbles, sheets, links, buttons, and form controls are ignored by the tap toggle.
